...
# AMOD 5430 final project
# Generates the six visualization groups used in the paper.
...
results_path <- "data/Results_sample.csv"
races_path <- "data/Races_sample.csv"
figure_dir <- "figures"
scatter_minimum <- 5
ranking_minimum <- 10

if (!file.exists(results_path) || !file.exists(races_path)) {
  stop("Run this script from the submission folder so it can find data/*.csv.")
}
...
race_metrics <- joined %>%
  ...
  mutate(plot_size = reported_finishers)
...
cat("\nDataset summary\n")
cat("Data: included anonymous sample\n")
