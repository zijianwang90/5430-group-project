...
The expected output is a reproducible exploratory analysis with clear static visualizations that explain marathon performance structure across demographic and race contexts.
...
The analysis will derive performance groups from finish time. The bands are mutually exclusive finish-time intervals, so each finisher belongs to exactly one band. Initial bands may include:

| Band | Finish-Time Interval | Interpretation |
|---|---:|---|
| Elite / near-elite | Under 2:45 | Highest-level finishers |
| Sub-3 | 2:45-2:59 | Competitive finishers |
| Sub-4 | 3:00-3:59 | Strong recreational finishers |
...
Note: `data/processed/`, `data/external/`, and extracted CSV files under `data/raw/` are working directories that are not tracked in Git (see `.gitignore`).
